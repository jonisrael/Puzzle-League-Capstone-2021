...
} catch {successfulFetch = false; console.log("fetch failed")}
try {
...
function CONTROL(event){
    if (running) {
        ...
        if (event.keyCode == 13 && dateTimeAPI.length != 0) { // enter
            ...
            running = true
            if(successfulFetch){board = makeOpeningBoard(extractTimeToIndex(dateTimeAPI))}
            else {board = generateOpeningBoard()}
