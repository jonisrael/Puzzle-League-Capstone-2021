...
for (let index=0; index<2; index++) {
    ...
    for (let c=0; c<COLS; c++) {
        for (let r=0; r<ROWS+2; r++) {
            boardEntry.x = c
            boardEntry.y = r
            boardEntry.color = board[c][r].color
            boardEntry.type = board[c][r].type
            boardInfo.push(boardEntry)
            boardEntry = {}
        }
    }
    data.push(boardInfo)
...
// console.log(databaseString)
// let databaseArray = databaseString.split(```)
// console.log(databaseArray.lengthS)



// // console.log(fs)
// fsLibrary.writeFile('database.json', JSON.stringify(database), (error) => {
...
function makeOpeningBoard(index) {
    fsLibrary.readFile("database.json", "utf8", (err, jsonString) => {
        if (err) {console.log("File read fail"); return}
